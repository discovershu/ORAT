...
if __name__ == "__main__":
    ...
    # savemat('./data/mnist_noise_data/mnist_sym_noise_{}.mat'.format(noise_ratio_value), mnist_noise_data_finished)

    ####cifar10
    X_train, Y_train, X_test, Y_test = get_data(dataset='cifar-10', noise_ratio=noise_ratio_value, asym=False)
    cifar_noise_data_finished = dict({'X_train':X_train, 'Y_train': Y_train, \
                                      'X_test': X_test, 'Y_test': Y_test})
    os.makedirs('../data/cifar_10_noise_data', exist_ok=True)
    savemat('../data/cifar_10_noise_data/cifar_10_noise_sym_{}_new.mat'.format(noise_ratio_value), cifar_noise_data_finished)

    ####mnist
    # X_train, Y_train, X_test, Y_test = get_data(dataset='mnist', noise_ratio=noise_ratio_value, asym=True)
    # mnist_noise_data_finished = dict({'X_train': X_train, 'Y_train': Y_train, \
    #                                   'X_test': X_test, 'Y_test': Y_test})
    # os.makedirs('../dataset/mnist_noise_data', exist_ok=True)
    # savemat('../dataset/mnist_noise_data/mnist_noise_asym_{}.mat'.format(noise_ratio_value),
    #         mnist_noise_data_finished)

    ####cifar100
    # X_train, Y_train, X_test, Y_test = get_data(dataset='cifar-100', noise_ratio=noise_ratio_value, asym=True)
    # cifar_noise_data_finished = dict({'X_train': X_train, 'Y_train': Y_train,
    #                                   'X_test': X_test, 'Y_test': Y_test})
    # os.makedirs('../data/cifar_100_noise_data', exist_ok=True)
    # savemat('../data/cifar_100_noise_data/cifar_100_noise_asym_{}.mat'.format(noise_ratio_value), cifar_noise_data_finished)
